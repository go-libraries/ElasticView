...
func HashHmac(baseStr string, key string) string {
	//hmac ,use sha1
...
func ValidateSignForMD5(r *http.Request, appKey string) error {
	...
	var baseStr = SortAndAppendMap(
		r.Form, "", true, "&", "=", ":"+appKey, "sign", "Sign", "act", "Act",
	)
	if !strings.EqualFold(MD5HexHash([]byte(baseStr)), sign) {
		baseStr = strings.Replace(baseStr, " ", "+", -1)
		if !strings.EqualFold(MD5HexHash([]byte(baseStr)), sign) {
			return fmt.Errorf("check sign failed:%s, %s", baseStr, MD5HexHash([]byte(baseStr)))
		}
	}
	return nil
}
...
func ProduceSignForMD5(form map[string][]string, appKey string) string {
	...
	baseStr = strings.Replace(baseStr, " ", "+", -1)
	return MD5HexHash([]byte(baseStr))
}
...
func SortAndAppendMap(value url.Values, strHd string, haveKey bool, strAnd, strEq, strAp string, excepts ...string) string {
	...
	for i, _ := range s {
		...
		if b {
			if haveKey {
				buf.WriteString(s[i] + strEq)
			}
			buf.WriteString(value[s[i]][0])
			b = false
			continue
		}
